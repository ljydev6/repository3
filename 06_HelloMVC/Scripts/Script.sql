...
AS
SELECT LEVEL, C.*
FROM (SELECT * FROM BOARD_COMMENT) C
	START WITH BOARD_COMMENT*/
SELECT *
FROM (SELECT * FROM BOARD_COMMENT WHERE BOARD_REF = 34 ORDER BY 1 DESC)C
	START WITH BOARD_COMMENT_LEVEL = 1
	CONNECT BY PRIOR BOARD_COMMENT_NO = BOARD_COMMENT_REF;

SELECT * FROM BOARD_COMMENT WHERE BOARD_REF = 34
	START WITH BOARD_COMMENT_LEVEL = 1
	CONNECT BY PRIOR BOARD_COMMENT_NO = BOARD_COMMENT_REF
